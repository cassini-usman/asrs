(function(jQuery) {
	...
	jQuery.fn.stickyNotes.deleteNoteById = function(note_id) {
		jQuery.each(jQuery.fn.stickyNotes.notes, function(index, note) {
			if (note.id == note_id) {
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		...
		if (jQuery.fn.stickyNotes.options.controls) {
			jQuery("#sticky-container").html('<div class="button" id="add_note">Add Note</div>');
	...
	jQuery.fn.stickyNotes.stopEditing = function(note_id) {
		...
		note.text = jQuery("#note-" + note_id).find('textarea').val().replace(/\n/g, '<br>');
		var _p_note_text = jQuery(document.createElement('p')).attr("id", "p-note-" + note_id)
							.html(note.text);
		jQuery("#note-" + note_id).find('textarea').replaceWith(_p_note_text);
		jQuery("#p-note-" + note_id).dblclick(function() {
	...
	jQuery.fn.stickyNotes.deleteNote = function(delete_button) {
		...
		jQuery.fn.stickyNotes.deleteNoteById(note_id);
	}

	jQuery.fn.stickyNotes.editNote = function(paragraph) {
		var note_id = jQuery(paragraph).parent().parent().attr("id").replace(/note-/, "");
		var textarea = jQuery(document.createElement('textarea')).attr("id", "textarea-note-" + note_id)
							.val(
								jQuery("#note-" + note_id)
								.find('p')
								.html().replace(/<br>/g, "\n")
							);
		jQuery("#p-note-" + note_id).replaceWith(textarea);
	...
	jQuery.fn.stickyNotes.renderNote = function(note) {
		...
		jQuery(_p_note_text).dblclick(function() {
			jQuery.fn.stickyNotes.editNote(this);
		});
	}
	...
	jQuery.fn.stickyNotes.createNoteBackground = function() {
		...
		if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
			background = jQuery(document.createElement('div')).addClass("background").html('<img src="images/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'images/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
		} else {
			background = jQuery(document.createElement('div')).addClass("background").html('<img src="images/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
		}
		return background;
